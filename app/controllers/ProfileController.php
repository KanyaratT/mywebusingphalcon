...
class ProfileController extends ControllerBase {
  ...
  public function indexAction(){
	  ...
	  $profileObj = User::findFirst($profileId) ; // update data
	  // $profileObj = new user() ; //this mean insert
	  $profileObj->username = $email ;
	  $profileObj->first_name = $firstname ;
	  $profileObj->picture=$photoUpdate ;
	  $profileObj->save() ;
	  //$profileObj->delete // to delete obj
   } 	 
	   else
   	   $profileId = $this->session->get('memberAuthen');
	   $profile=user::findFirst($profileId); //user is a class name can be use as User or user. 
